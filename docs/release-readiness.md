...
generator/runtime performance gate needs a formal Pure Ruby comparison, and
the published ten-case error comparison has not received independent
third-party review.
...
| Generator and parser performance at least racc | Pending | The previous native-Racc diagnostic is not formal evidence for the Pure Ruby comparison contract |
...
1. Repeat the public-gem measurements with Ibex and Racc both using their Pure
   Ruby runtimes, then optimize any row that remains slower than racc.
