# ADR 0038: Resolve canonical grammar fragments inside one source root
...
`expansion.include_chain`. ADR 0039 later attaches lossless rule documentation before resolution; fragment documentation follows
the same merge, location, identity, and deep-freeze rules and populates version-2 symbol and production fields. Version-1
production serialization still omits all version-2 documentation and expansion fields. ADR 0040 later treats parameterized
