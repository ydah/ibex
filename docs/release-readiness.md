...
| Generator and parser performance at least racc | Fail | Generated size and reuse allocations pass, but all nine external timing rows and all three new-instance allocation rows fail |
...
The formal artifact was collected from clean revision `53aa4ab` with ten
...
verified to use its native backend. Ratios below are Ibex divided by racc, so
values at or below 1.0 pass.
...
code-generation work. These are evidence for the release decision, not
portable scores.
