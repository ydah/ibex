...
| --- | ---: | --- | --- | --- |
| Grammar IR | v2 for declaration-free source; v3 for explicit parser contracts and migration output | v2, v3 | v2 to v3; downgrade is refused | published v2-v3 schemas stay closed |
| Automaton IR | v2 from Grammar IR v2; v3 from Grammar IR v3 and migration output | v2, v3 | v2 to v3; embedded Grammar IR is upgraded and its digest is recalculated | published v2-v3 schemas stay closed |
| Lexer IR | v1 | v1 | none | the closed v1 schema changes only through a new version |

`Ibex::IR::Migration.to_version` is the only supported upgrade path. Version-2
effective parser algorithm, entry construction, and CST trivia that cannot be
recovered are recorded as unavailable instead of being invented from current
defaults. Migration is idempotent at its target version.
