...
| --- | ---: | --- | --- | --- |
| Grammar IR | v2 for declaration-free source; v3 for explicit parser contracts and migration output | v1, v2, v3 | v1 to v2/v3 and v2 to v3; downgrade is refused | published v1-v3 schemas stay closed |
| Automaton IR | v2 from Grammar IR v2; v3 from Grammar IR v3 and migration output | v1, v2, v3 | v1 to v2/v3 and v2 to v3; embedded Grammar IR is upgraded and its digest is recalculated | published v1-v3 schemas stay closed |
...
`Ibex::IR::Migration.to_version` is the only supported upgrade path. Version-1
metadata and version-2 effective parser algorithm, entry construction, and CST
trivia that cannot be recovered are recorded as unavailable instead of being
invented from current defaults. Migration is idempotent at its target version.
Declaration-free normalized source still writes v2, preserving generated bytes.
Grammar IR v3 carries a required root `parser_contract` whose individual fields
are either explicit with source locations or explicitly unspecified. Automaton
IR v3 embeds Grammar IR v3, includes that contract in `grammar_digest`, and
