...
`test:no_exec` holds the static side of the
[execution trust matrix](../README.md#execution-trust-matrix): grammar and IR
tools must not execute parser actions, generated lexer actions, or user-code
sections. `test/runtime/cst_incremental_test.rb` holds the generated
syntax-only side with independent sentinels: lexer actions execute while parser
production actions do not. These guarantees do not make a generated parser a
sandbox; test generated files only when their embedded Ruby is trusted.
