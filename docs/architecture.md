...
| Field | Meaning |
|---|---|
| `ibex_ir`, `schema_version` | `"grammar"`, `2` or `3` |
...
Normalized grammars use version 2 for declaration-free source and add explicit nullable metadata:
...
Grammar IR v3 adds one closed, root-owned `parser_contract`. Its `algorithm`, `entries`, and `cst_trivia` members each carry
...
`Serialize.load` and `Validator.validate` accept Grammar IR v2 and v3 and preserve the loaded shape. Declaration-free source
normalization continues to write version 2, while a source `parser` declaration writes version 3. This preserves existing
generated bytes when the feature is absent.
`IR::Migration.to_version` upgrades v2 to v3. A v3 migration marks historical effective algorithm, entry
...
Automata built from Grammar IR v2 use version 2. Automaton IR v3 is selected when its input grammar is v3, embeds that exact
...
`unknown` as the current `shared` built-in. Version-2 and version-3 automata remain loadable, validatable, and byte-stable.
...
the original source and rejects algorithm/entry construction flags because its tables are already built. A v3 generation
