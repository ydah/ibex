  ...
  module LSP
    ...
    module NavigationHandlers
      ...
      def definition(raw_params)
        with_index(raw_params) { |index, path, position| index.definition(path, position) } #: Array[lsp_object]
      end
      ...
      def prepare_rename(raw_params)
        with_index(raw_params) { |index, path, position| index.prepare_rename(path, position) } #: lsp_object?
      end
      ...
      def rename(raw_params)
        ...
        with_index(params) { |index, path, position| index.rename(path, position, new_name) } #: lsp_object
      end
